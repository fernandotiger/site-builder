...
deployRouter.post("/add", protect, async (req, res) => {
    ...
    const result = await deployProject(projectId.trim(), userId).catch((err) => ({
        success: false,
 ...
 * }
 */
deployRouter.delete("/delete", protect, async (req, res) => {
    const { projectId, deleteFiles = true } = req.body;
    const userId = req.userId;
    if (!userId) {
        res.status(401).json({ message: 'Unauthorized' });
        return;
    ...
    const result = await undeployProject(projectId.trim(), userId, deleteFiles).catch((err) => ({
        success: false,
        message: err instanceof Error ? err.message : String(err),
    }));
    res.status(result.success ? 200 : 500).json(result);
