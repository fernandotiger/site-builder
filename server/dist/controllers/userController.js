...
export const getPlanData = async (userId) => {
...
export const createUserProject = async (req, res) => {
    const userId = req.userId;
    const modelName = await getAiModelNameForUser(userId);
    const userPlan = await getPlanData(userId);
    let projectId = null;
    try {
        ...
        try {
            ...
            await sendProjectCompletedEmail(user?.email, project.name, projectUrl);
        }
        catch (err) {
...
export const purchaseCredits = async (req, res) => {
    try {
        const plans = {
            basic: { credits: 500, amount: 70 },
            pro: { credits: 500, amount: 70 },
            enterprise: { credits: 2500, amount: 250 },
        };
        const userId = req.userId;
        let { planId } = req.body;
...
`;
function canCreateNewProject(user, userPlan) {
    if (!user)
        return true;
    const projectLimits = {
        'basic': 51,
...
function getGDPRPrompt(addGDPRInfo) {
    if (addGDPRInfo) {
