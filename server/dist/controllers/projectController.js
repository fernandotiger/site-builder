...
export const makeRevision = async (req, res) => {
    ...
    const modelNameRevision = await getAiModelNameRevisionForUser(userId);
    try {
        ...
        const promptCheckerResponse = await openai.chat.completions.create({
            model: process.env.OPENROUTER_MODEL_NAME_QESTIONS,
            messages: [
                {
                    role: 'system',
        ...
        if (customDomain != null && customDomain.trim() !== '') {
            ...
            existingDomain = await prisma.websiteProject.findFirst({
                where: {
                    ...
                    NOT: { id: projectId } // exclude the current project
                }
            });
        }
        if (existingDomain) {
