 ...
 */
const ROLE_HIERARCHY = ["basic", "pro", "enterprise"];
// ─── Core helper ─────────────────────────────────────────────────────────────
/**
...
export async function getUserRole(userId) {
    ...
    const planId = latestPaidTransaction.planId;
    // Safety: ensure planId is actually a valid role
    if (!ROLE_HIERARCHY.includes(planId)) {
        console.warn(`User Role: unknown planId "${planId}" for user ${userId}, defaulting to "basic"`);
        return "basic";
    }
    return planId;
}
 ...
 */
export async function userHasAtLeastRole(userId, minimumRole) {
    const userRole = await getUserRole(userId);
